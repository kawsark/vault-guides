...
# key ring datasource
data "google_kms_key_ring" "key_ring_ds" {
  name     = var.key_ring
...
# Create a crypto key for the key ring
 ...
 count = var.create_key ? 1 : 0
   ...
   key_ring = var.create_keyring ? "${google_kms_key_ring.key_ring[0].self_link}" : data.google_kms_key_ring.key_ring_ds.self_link
   rotation_period = "100000s"
 }

# Add the service account to the Keyring
resource "google_kms_key_ring_iam_binding" "vault_iam_kms_binding" {
  key_ring_id = var.create_keyring ? "${google_kms_key_ring.key_ring[0].id}" : "${var.gcloud-project}/${var.keyring_location}/${var.key_ring}"
